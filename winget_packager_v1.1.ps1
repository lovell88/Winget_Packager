...
#Welcome message
Write-host "Welcome to the winget script packaging tool! `nMore info at my github: https://github.com/lovell88/Winget_Packager
...
while (-not ($search -in $validValues)) {
    ...
    if ($search -like "y*") {
        ...
        elseif (!($wingetCheck -match "Microsoft.DesktopAppInstaller")) {
            ...
            write-host "Enter number below:" -ForegroundColor yellow
            ...
            elseif ($wingetChoice -eq "2") {
                Write-Host "`nThis script will attempt to install winget.`nMore information on this script can be found here: `nhttps://github.com/asheroto/winget-install" -foregroundcolor red
...
# Validates the resource ID and displays installer information if desired

Write-Host "`nWould you like VALIDATE your resource ID and review the default installer information for this package?"
Write-Host "PLEASE NOTE: This command will only work if Winget is installed and able to search the winget repo." -ForegroundColor Cyan

write-host "[Y] Yes  [N] No" -ForegroundColor yellow
$checkInstaller = Read-Host

while (-not ($checkInstaller -in $validValues)) {
    Write-host "`nYour response needs to be `"Yes`", `"Y`", `"No`" or `"N`" (case-insensitive). Please enter it again:" -ForegroundColor Magenta
    $checkInstaller = read-host

}

if ($checkInstaller -like "y*") {


    Write-Host "`nIMPORTANT: This will only validate if the resource ID exists. It's the user's responsibility to ensure `nit matches the intended program." -ForegroundColor Red  
    Write-Host "`nWould you like to continue with validation?"
    write-host "[Y] Yes  [N] No exit validation and return to script" -ForegroundColor yellow
        ...
        if ($continue -like "y*") {
            ...
            $wingetCheck = Get-AppPackage *Microsoft.DesktopAppInstaller* | Out-String
                ...
                if ($wingetCheck -match "Microsoft.DesktopAppInstaller") {
                    ...
                    write-host "`nChecking the winget repo for a match. Please wait..." -ForegroundColor Yellow  

                    $packageCheck = winget search $resourceID -e | Out-String
                    $installerVersion = winget show $resourceID | Select-String -Pattern "^Version" | Out-string
                    $installerInfo = winget show $resourceID | Select-Object -Last 3 | Out-string
                    
                      

                    if ($packageCheck -match "No package found") {
                        $notFound = "true"
                                    }

                    while ($notFound -eq "true") {
                        write-host "`nSorry! It looks like there was no match in the winget repo for that resource ID." -ForegroundColor Red
                        Write-Host "`nWould you like to search the repo for the correct resource ID?"
                        write-host "[Y] Yes  [N] No to exit script" -ForegroundColor yellow
                        ...
                        if ($search -like "y*") {
                            ...
                            $wingetResult = winget search $resourceID -e | Out-String
                            
                            if ($wingetResult -match "No package found") {
                                $notFound = "true"
                            }
                            ...
                            Write-Host "Better luck next time! Have a great day!"
                        }

                    }
                    Write-Host "`nCONGRATULATIONS! We were able to validate your resource ID!" -ForegroundColor Magenta    
                    Write-Host "`nInstaller information for $resourceID`:"
                    Write-Host "`n$installerVersion`n$installerInfo" -ForegroundColor Yellow
                    
                }

                else {
                    Write-Host "`nSorry! Winget is not installed, and this script is unable to find what you desire." -ForegroundColor Red
                }
        }
        else {
            Write-Host "`nExiting validation phase and returning to normal script flow." -ForegroundColor magenta
        }
    }
...
Write-Host "`nWould you like add arguments to be passed to the installation of the app?"
write-host "[Y] Yes  [N] No" -ForegroundColor yellow

$checkArguments = Read-Host

while (-not ($checkArguments -in $validValues)) {
    Write-host "`nYour response needs to be `"Yes`", `"Y`", `"No`" or `"N`" (case-insensitive). Please enter it again:" -ForegroundColor Magenta
    $checkArguments = read-host

}

    if ($checkArguments -like "y*") {
        Write-Host "`nPLEASE NOTE: Make sure your arguments are supported on the default installer used by winget.
                    `nIf you didn't review the install information earlier, please restart the script or `nrun `"winget show <RESOURCEID>`" separately.
                    `nIf the the default install package is not what you're expecting, search the winget `nmanifests at https://github.com/microsoft/winget-pkgs/tree/master/manifests" -ForegroundColor Cyan

        Write-Host "`nWhat are the arguments?"
        Write-Host "NOTE: Make sure any quotes are the proper unicode equivalents." -ForegroundColor red
        
        $arguments = read-host

        Write-Host "`nThanks! You can modify these at any time in the install$name.ps1 file." -ForegroundColor Yellow


    }

    else {
        $arguments = "No arguments were added."
    }
...
Arguments: $arguments `n" -ForegroundColor cyan

Write-Host "NOTE: This will create scripts for the installation, detection, and uninstall of your $name package." -ForegroundColor Yellow
...
#
# You can also test the scope by simply running the install ps1 file that 
# this script creates. You will see if it fails then.
#
...
## Sets the variable for programName to $resourceID and programArguments to $arguments. DO NOT DELETE. Script creation will not work properly without this

$ProgramName = '$ProgramName'
$programArguments = '$programArguments'


Set-Content -path $installSave -value "$ProgramName = `"$resourceID`"`n$programArguments= ' $arguments'"
...
## format like this https://stackoverflow.com/questions/76881152/powershell-add-set-content-doesnt-add-variables
    ...
    if ($wingetCheck -match "Microsoft.DesktopAppInstaller") {
        ...
        winget install --exact --id $ProgramName --silent --accept-package-agreements --accept-source-agreements --custom $programArguments      
    }
...
$wingetCheck = Get-AppPackage *Microsoft.DesktopAppInstaller* | Out-String
    ...
    if ($wingetCheck -match "Microsoft.DesktopAppInstaller") {
        ...
        winget uninstall --exact --id $ProgramName --silent --accept-package-agreements --accept-source-agreements --scope=user
...
# =========================================================================
#
# Creates the DETECT script file. Includes error handling for if winget
...
$wingetCheck = Get-AppPackage *Microsoft.DesktopAppInstaller* | Out-String
    ...
    if ($wingetCheck -match "Microsoft.DesktopAppInstaller") {
        ...
        $wingetPrg_Existing = & winget list --id $ProgramName --exact --accept-source-agreements
        
        if ($wingetPrg_Existing -like "*$ProgramName*"){
            Write-Host "Found $ProgramName!"
            exit 0
        }

        elseif (!($wingetPrg_Existing -like "*$ProgramName*")) {
            Write-Error "Unable to find $ProgramName"
            exit 1
        }
    }
...
# =========================================================================
#
# Runs the content prep tool to package the created powershell 
# scripts in an intunewin file.
#
# NOTE: Requires that the content prep tool not be renamed from
# "IntuneWinAppUtil.exe" for this to work. 
#
# =========================================================================
